...
describe("Tokens Contract", function () {
      ...
      this.tokens.mintingFee().then((fee: BigNumber) => {
        return this.tokensAs(this.OWNER)
          ...
          .then(async (_: any) => {
            ...
            await this.tokensAs(this.OWNER).setAdmin(
              _nextNFTIndex,
              this.ADMIN.address
            );
  ...
  describe("Deployment", function () {
    ...
    it("should set _uri to value in env", async function () {
      ...
      expect(await this.tokens.minimumMiningYield()).to.be.equal(
        minimumMiningYield
      );
      ...
      expect(await this.tokens.miningIntervalSeconds()).to.be.equal(
    ...
    it("should support all expected ERC165 interfaces", async function () {
      ...
      expect(await this.tokens.supportsInterface(IID_IACCESS_CONTROL)).to.be
        .true;
      ...
      expect(await this.tokens.supportsInterface(IID_IERC20Metadata)).to.be
        .true;
      expect(await this.tokens.supportsInterface(IID_IERC1155MetadataURI)).to.be
        .true;
    });
    ...
    it("should revert on fallback", async function () {
      ...
      const fakeDemoContract = new ethers.Contract(
        this.tokens.address,
        [
          ...this.tokens.interface.fragments,
    ...
    it("should revert on internal function errors", async function () {
      ...
      let initialMiningYield = ethers.BigNumber.from(16).mul(
        BigNumber.from(10).pow(18)
      );
      let minimumMiningYield = ethers.BigNumber.from(1).mul(
        BigNumber.from(10).pow(18)
      );
      let maxTotalSupply = ethers.BigNumber.from(40).mul(
        BigNumber.from(10).pow(18)
      );
  ...
  describe("mineSCI", function () {
    it("should mine SCI tokens as expected", async function () {
      ...
      for (let i = 0; i < 32; i++) {
        ...
        let lastTime = BigNumber.from(await time.latest());
        let interval = BigNumber.from(
          await this.tokensAs(this.ANYONE).miningIntervalSeconds()
        );
        time.setNextBlockTimestamp(lastTime.add(interval).add(1));

        await this.tokensAs(this.ANYONE).mineSCI(
          solution,
          this.ANYONE.address,
          {
            value: BigNumber.from(fee).toString(),
        ...
        expect(await this.tokens.lastMiningSolution()).to.equal(
          ethers.utils.hexlify(solution)
        );
      }
      expect(await this.tokens.miningGeneration()).to.equal(5);
      // we mined 32 times, generation 5 yield should be decreased by 4^5 = 1024
      expect(await this.tokens.miningYield()).to.equal(
        firstMiningYield.div(1024)
      );
    }).timeout(10000);
    ...
    it("should hit a minimum miningYield and stop at maxTotalSupply as expected", async function () {
      ...
      await fastTokensAsCEO.grantRole(
        await fastTokensAsCEO.CFO_ROLE(),
        this.CFO.address
      );
      ...
      while (true) {
        ...
        if (i == 0 || generation != thisG) {
          ...
          if (expected.lt(minMiningYield)) {
            expect(await fastTokensAsCEO.miningYield()).to.equal(
              minMiningYield
            );
            break;
          }
        ...
        let lastTime = BigNumber.from(await time.latest());
        let interval = BigNumber.from(
          await fastTokensAsCEO.miningIntervalSeconds()
        );
        time.setNextBlockTimestamp(lastTime.add(interval).add(1));
      ...
      let remainingSupply = maxTotalSupply.sub(
        await fastTokensAsCEO.totalSupply()
      );
      let remainingCalls = remainingSupply
        .div(await fastTokensAsCEO.miningYield())
        .toNumber();
      // mine until we hit minimum
    ...
    it("should reject with an incorrect solution", async function () {
      ...
      expect(receipt.events?.filter((x: Event) => x.event == "MiningFeeSet")).to
        .not.be.null;
  ...
  describe("superadminMintNFT", function () {
    ...
    it("should mint an NFT with arbitrary values as the SUPERADMIN", async function () {
      ...
      expect((await this.tokens.scienceNFTs(tokenId + 1)).createdAt).to.equal(
        createdAt
      );
      expect((await this.tokens.scienceNFTs(tokenId + 1)).status).to.equal(
        status
      );
    });
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = allSigners.filter(
        (s) => s.address != this.SUPERADMIN.address
      );
  ...
  describe("mintNFT", function () {
    ...
    it("should revert minting an NFT without fee payment", async function () {
      ...
      const receipt = await tx.wait();
      expect(receipt.events?.filter((x: Event) => x.event == "MintingFeeSet"))
        .to.not.be.null;
    ...
    it("should mint an NFT with expected status", async function () {
      ...
      let expectedStatus =
        (await this.tokens.FULL_BENEFIT_FLAG()) |
        (await this.tokens.UNSET_FULL_BENEFIT_FLAG());
      expect((await this.tokens.scienceNFTs(tokenId + 1)).status).to.be.equal(
        expectedStatus
      );
    });
    ...
    it("should mint NFT with correct minting fee for full parameter version", async function () {
      ...
      await this.tokensAs(this.OWNER)[
        "mintNFT(bytes32,uint192,address,address,address)"
      ](
        ADMIN_HASH_1,
    ...
    it("should emit NFTUpdated event on successful minting", async function () {
      let tokenId = await this.mintNFTWithOwnerAndAdmin();
      let status = 11;
      let tx = await this.tokensAs(this.OWNER)[
        "mintNFT(bytes32,uint192,address,address,address)"
      ](
        ADMIN_HASH_1,
        BigNumber.from(status),
      ...
      expect(receipt.events?.filter((x: Event) => x.event == "NFTUpdated")).to
        .not.be.null;
      expect(
        receipt.events?.filter(
          (x: Event) => x.event == "AdminContentNodeCreated"
        )
      ).to.not.be.null;

      let NFTUpdatedEvent = receipt.events.find(
        (e: any) => e.event == "NFTUpdated"
      );
      expect(NFTUpdatedEvent.args.tokenId).to.equal(tokenId + 1);
      expect(NFTUpdatedEvent.args.status).to.equal(status);
      expect(NFTUpdatedEvent.args.owner).to.equal(
        await this.OWNER.getAddress()
      );
      expect(NFTUpdatedEvent.args.admin).to.equal(
        await this.ADMIN.getAddress()
      );
      expect(NFTUpdatedEvent.args.beneficiary).to.equal(
        await this.BENEFICIARY.getAddress()
      );
  ...
  describe("safeBatchTransferFrom", function () {
    it("should transfer several NFTs from sender to receiver", async function () {
      ...
      expect(
        await this.tokens.balanceOfBatch(ownerRepeated, NFT_INDEXES)
      ).to.deep.equal(allOnes);
      expect(
        await this.tokens.balanceOfBatch(anyoneRepeated, NFT_INDEXES)
      ).to.deep.equal(allZeroes);
      ...
      expect(
        await this.tokens["balanceOf(address,uint256)"](this.OWNER.address, 0)
      ).to.equal(5);
  ...
  describe("batch approvals", function () {
    it("should approve a batch of operators", async function () {
      ...
      expect(
        await this.tokens.isApprovedForAll(this.ANYONE.address, operators[0])
      ).to.deep.equal(true);
      expect(
        await this.tokens.isApprovedForAll(this.ANYONE.address, operators[1])
      ).to.deep.equal(false);
      let anyoneRepeated = approvals.map((_) => this.ANYONE.address);
      let contractApprovals = await this.tokensAs(
        this.ANYONE
      ).isApprovedForAllBatch(anyoneRepeated, operators);
  ...
  describe("withdrawFees", function () {
    it("should withdraw minting fees as the CFO", async function () {
      ...
      expect(await this.ANYONE.getBalance()).to.equal(
        initialAnyoneBalance.add(mintingFee)
      );
    });
    ...
    it("should revert on insufficient funds", async function () {
      ...
      await this.toRevert(async () => {
        await this.tokensAs(this.CFO).withdraw(
          this.ANYONE.address,
          mintingFee.add(1)
        );
  ...
  describe("setMintingFee", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = allSigners.filter((s) => s.address != this.CFO.address);
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).setMintingFee(99);
      let m = "Only CFO";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
  ...
  describe("setMiningFee", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = allSigners.filter((s) => s.address != this.CFO.address);
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).setMiningFee(99);
      let m = "Only CFO";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
  ...
  describe("setDifficulty", function () {
    it("should set difficulty as the CFO", async function () {
      ...
      const receipt = await tx.wait();
      expect(receipt.events?.filter((x: Event) => x.event == "DifficultySet"))
        .to.not.be.null;
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = allSigners.filter((s) => s.address != this.CFO.address);
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).setDifficulty(99);
      let m = "Only CFO";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
  ...
  describe("setMiningInterval", function () {
    it("should set mining interval as the CFO", async function () {
      ...
      const receipt = await tx.wait();
      expect(
        receipt.events?.filter((x: Event) => x.event == "MiningIntervalSet")
      ).to.not.be.null;
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = allSigners.filter((s) => s.address != this.CFO.address);
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).setMiningInterval(99);
      let m = "Only CFO";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
  ...
  describe("changeAdmin", function () {
    it("should set the ADMIN address to the OWNER address for a new NFT", async function () {
      ...
      expect(await this.tokens.adminOf(tokenId + 1)).to.equal(
        this.OWNER.address
      );
    });
    ...
    it("should revert for an NFT that is not minted", async function () {
      let tokenId = await this.mintNFTWithOwnerAndAdmin();
      await this.toRevert(async () => {
        await this.tokensAs(this.ADMIN).setAdmin(
          tokenId + 1,
          this.ANYONE.address
        );
      }, "Invalid NFT");
    ...
    it("should revert as any other role", async function () {
      ...
      let allSigners = await ethers.getSigners();
      let notAllowed = allSigners.filter(
        (s) => s.address != this.ADMIN.address
      );
      let f = async (s: SignerWithAddress) =>
  ...
  describe("changeBeneficiary", function () {
    it("should set the BENEFICIARY address to the OWNER address for a new NFT", async function () {
      ...
      expect(await this.tokens.beneficiaryOf(tokenId + 1)).to.equal(
        this.OWNER.address
      );
    });
    ...
    it("should revert for an NFT that is not minted", async function () {
      let tokenId = await this.mintNFTWithOwnerAndAdmin();
      await this.toRevert(async () => {
        await this.tokensAs(this.ADMIN).setBeneficiary(
          tokenId + 1,
          this.ANYONE.address
        );
      }, "Invalid NFT");
    ...
    it("should change the BENEFICIARY address as the ADMIN", async function () {
      ...
      expect(await this.tokens.beneficiaryOf(tokenId)).to.equal(
        this.ANYONE.address
      );
    });
  ...
  describe("appending content", function () {
    it("should set the owner and admin hashes correctly for a new NFT", async function () {
      ...
      expect((await this.tokens.scienceNFTs(tokenId + 1)).adminHash).to.equal(
        OWNER_HASH_1
      );
    });
    ...
    it("should revert in getAdjacentContent for invalid conditions", async function () {
      ...
      await this.toRevert(async () => {
        await this.tokens.getAdjacentContent(
          tokenId + 1,
          ADMIN_HASH_1,
          ENUM_ADMIN_CONTENT
        );
    ...
    it("should revert in appendContent for invalid conditions", async function () {
      ...
      expect((await this.tokens.scienceNFTs(tokenId)).adminHash).to.equal(
        ADMIN_HASH_1
      );
      await this.toRevert(async () => {
        await this.tokensAs(this.ADMIN).appendContent(
    ...
    it("should emit events as expected for OwnerContentNodeCreated", async function () {
      ...
      expect(
        r1.events?.filter((x: Event) => x.event == "OwnerContentNodeCreated")
      ).to.not.be.null;
      ...
      expect(
        r2.events?.filter((x: Event) => x.event == "OwnerContentNodeCreated")
      ).to.not.be.null;
      ...
      expect(
        r3.events?.filter((x: Event) => x.event == "OwnerContentNodeCreated")
      ).to.not.be.null;
    ...
    it("should add new ownerHash values as the OWNER", async function () {
      ...
      expect(
        receipt.events?.filter(
          (x: Event) => x.event == "OwnerContentNodeCreated"
        )
      ).to.not.be.null;
    ...
    it("should append to the admin list as the SUPERADMIN", async function () {
      ...
      let appendAdmin = async (data, prev) => {
        ...
        let [p, n] = await this.tokens.getAdjacentContent(
          tokenId,
          data,
          ENUM_ADMIN_CONTENT
        );
      ...
      [p, n] = await this.tokens.getAdjacentContent(
        tokenId,
        OWNER_HASH_2,
        ENUM_OWNER_CONTENT
      );
  ...
  describe("Retracted Flag", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).blocklist(tokenId, true);
      let m = "Only SUPERADMIN";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
  ...
  describe("Full Benefit Flag", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let allSigners = await ethers.getSigners();
      let notAllowed = allSigners.filter(
        (s) => s.address != this.OWNER.address
      );
      let f = async (s: SignerWithAddress) =>
  ...
  describe("UNSET_FULL_BENEFIT_FLAG", function () {
    it("should set UNSET_FULL_BENEFIT_FLAG to true for a new NFT", async function () {
      ...
      expect(await this.tokens.willUnsetFullBenefit(tokenId + 1)).to.equal(
        true
      );
    });
  });
  ...
  describe("BRIDGED_FLAG", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).withdrawFromContract(
          tokenId,
          this.BRIDGE.address
        );
  ...
  describe("a signer acting as a BRIDGE", function () {
    ...
    it("should withdraw and restore an NFT with arbitrary values as the BRIDGE", async function () {
      ...
      let status = BigNumber.from(
        0xfffffff | (await this.tokens.BRIDGED_FLAG())
      );
      await this.tokensAs(this.BRIDGE).restoreToContract(
        tokenId,
        status,
      ...
      let expectedStatus = BigNumber.from(
        0xfffffff & ~(await this.tokens.BRIDGED_FLAG())
      );
      ...
      expect((await this.tokens.scienceNFTs(tokenId)).status).to.equal(
        expectedStatus
      );
      expect((await this.tokens.scienceNFTs(tokenId)).createdAt).to.equal(
        createdAt
      );
    ...
    it("should revert as any other role", async function () {
      ...
      let allSigners = await ethers.getSigners();
      let notAllowed = allSigners.filter(
        (s) => s.address != this.BRIDGE.address
      );
      let f = async (s: SignerWithAddress) =>
  ...
  describe("reportMarketplaceSale", function () {
    it("should receive from MARKETPLACE", async function () {
      ...
      expect(receipt.events?.filter((x: Event) => x.event == "NFTUpdated")).to
        .not.be.null;
      expect(receipt.events?.filter((x: Event) => x.event == "MarketplaceSale"))
        .to.not.be.null;
      ...
      let marketplaceSaleEvent = receipt.events.find(
        (e: any) => e.event == "MarketplaceSale"
      );
    ...
    it("should revert as any other role", async function () {
      ...
      let notAllowed = await asyncFilter(
        allSigners,
        async (s: SignerWithAddress) =>
          !(await this.tokens.hasRole(marketplaceRole, s.address))
      );
      let f = async (s: SignerWithAddress) =>
  ...
  describe("setStatus", function () {
    ...
    it("should revert as any other role", async function () {
      ...
      let f = async (s: SignerWithAddress) =>
        await this.tokensAs(s).setStatus(tokenId, status);
      let m = "Only SUPERADMIN";
      expect(await this.checkAllRoles(notAllowed, f, m));
    });
  });
