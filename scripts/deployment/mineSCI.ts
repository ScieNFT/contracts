...
export async function mineSCI(
  tokensAddress: string,
  maximumMiningOperations: number
) {
  if (!tokensAddress) {
  ...
  while (miningIterations < maximumMiningOperations) {
    try {
      ...
      console.log(
        `[${Date.now()}] mined ${miningYield.div(
          BigNumber.from(10).pow(18)
        )} SCI to ${Signers.CFO.address} (${
          miningIterations + 1
        } of ${maximumMiningOperations})`
      );
      miningIterations++;
      ...
      console.log(
        `[${Date.now()}] waiting ${
          msecToWait / 1000
        } seconds until next mining attempt`
      );
  ...
  let balance = await Contracts.tokens["balanceOf(address)"](
    Signers.CFO.address
  );
  console.log(
