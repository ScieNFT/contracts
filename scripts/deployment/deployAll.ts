...
async function main() {
  ...
  for (let i: number = 2; i <= 10; i++) {
    ...
    console.log(
      `Checking DEPLOYING_MNEMONIC child balance ${i} @ ${child.address}`
    );
    let addressGas;
    try {
    ...
    console.log(
      `DEPLOYING_MNEMONIC child ${i} @ ${child.address} has ${addressGas} gas`
    );
    if (addressGas?.gt(0)) {
      console.log(`harvesting gas from ${i} @ ${child.address}`);
      let gasPrice = Math.round(
        (await child.provider.getGasPrice()).toNumber() * 1.02
      );
  ...
  Signers.SUPERADMIN = Wallet.fromMnemonic(m, `m/44'/60'/0'/0/0`).connect(
    provider
  );
  ...
  await tx.wait();
  console.log(
    `transferred ${gasAmount} to SUPERADMIN @ ${Signers.SUPERADMIN.address}`
  );

  console.log(
    `**        CEO @ ${Signers.CEO.address} has ${gasBalanceCEO} gas`
  );
  console.log(
    `**        CFO @ ${Signers.CFO.address} has ${gasBalanceCFO} gas`
  );
  console.log(
    `** SUPERADMIN @ ${Signers.SUPERADMIN.address} has ${gasBalanceSUPERADMIN} gas`
  );
  ...
  let balanceBefore = await Contracts.tokens["balanceOf(address)"](
    Signers.CFO.address
  );
  ...
  console.log(
    `SUPERADMIN @ ${Signers.SUPERADMIN.address} has ${scale(
      balanceSUPERADMIN
    ).toString()} SCI`
  );
